...
def make_dataset(dataset, data_type='train'):
    ...
    for idx in tqdm(range(dataset.shape[0])):
        ...
        sub_log = log_df[(log_df['sn'] == sn) & (log_df['time_ts'] <= fault_time_ts) & (fault_time_ts - log_df['time_ts'] <= 60 * 60 * 4)]
        ...
        data_tmp = get_w2v_feature(sub_log)
        data.update(data_tmp)

        data_tmp = get_feature(sub_log)
        data.update(data_tmp)

        data_tmp = get_tfidf_feature(sub_log)
        data.update(data_tmp)

        sub_log = sub_log.tail(5)

        log_cnt1.append(sub_log.shape[0])
        ret.append(data)
    print(np.mean(log_cnt1))
    return ret
