...
# df_train2 = pd.read_csv('train_new.csv')
# df_test2 = pd.read_csv('test_new.csv')
...
def cat_model_train(x_train, y_train, x_val, y_val, mode='multiclass'):
    ...
    model = lgb.LGBMClassifier(
        objective='multiclass' if mode == 'multiclass' else 'binary',
        n_estimators=10000,
...
for fold, (tr_ind, val_ind) in enumerate(folds.split(df_train, df_train['label'], df_train['sn'])):
    ...
    for i in range(val_proba.shape[0]):
        if np.argmax(val_proba[i]) == 0:
            proba = model2.predict_proba(x_val1)[i]
            val_pred.append(np.argmax(proba))
        else:
