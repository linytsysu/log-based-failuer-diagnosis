...
counter = Counter()
for value in log_df['msg_lower'].values:
    counter.update(value.split())
word2token = dict(counter)
cnt = 0
for k in word2token.keys():
    word2token[k] = cnt
    cnt += 1
...
for idx, row in tqdm(label_df.iterrows()):
    ...
    df = df.tail(4).copy()
    tr_data.append([sn, ' '.join(df['msg_lower'].values), label])
...
for idx, row in tqdm(submit_df.iterrows()):
    ...
    df = df.sort_values(by='time_ts').reset_index(drop=True)
    df = df.tail(4).copy()
    te_data.append([sn, ' '.join(df['msg_lower'].values)])
...
for idx, row in tqdm(submit_df_b.iterrows()):
    ...
    df = df.sort_values(by='time_ts').reset_index(drop=True)
    df = df.tail(4).copy()
    te_data_b.append([sn, ' '.join(df['msg_lower'].values)])
...
def load_data(data_list):
    ...
    for l in data_list:
        ...
        a = [word2token[i] for i in a.split(' ') if len(i) > 0]
